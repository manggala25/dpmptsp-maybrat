    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="font-weight-bolder text-info text-gradient">Edit Halaman Kontak</h3>
                
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="formEditData{{ $item->id }}" action="{{ route('menukontak.update', ['menukontak' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
