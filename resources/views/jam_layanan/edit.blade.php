        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formEditData{{ $item->id }}" action="{{ route('jam_layanan.update', ['jam_layanan' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <p class="text-danger">Mohon edit ulang semua masukan jika ingin mengedit!</p>
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" class="form-control" id="jam_buka" name="jam_buka" value="{{ $item->jam_buka }}">
                    </div>
                    <div class="form-group">
                        <label for="jam_tutup" class="col-form-label">Waktu Selesai:</label>
                        <input type="time" class="form-control" id="jam_tutup" name="jam_tutup" value="{{ $item->jam_tutup }}">
                    </div>
                    <div class="form-group">
                        <label class="col-form-label">Status fungsi:</label><br>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="status" id="status_buka" value="buka" {{ $item->status == 'buka' ? 'checked' : '' }}>
                            <label class="form-check-label" for="status_buka">buka</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="status" id="status_libur" value="libur" {{ $item->status == 'libur' ? 'checked' : '' }}>
